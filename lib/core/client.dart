...
class ApiClient {
  final Dio dio = Dio(
    BaseOptions(
      baseUrl: 'http://172.15.134.245:8888/api/v1',
  ...
  Future<dynamic> fetchTopChef() async {
    var response = await dio.get('/top-chef/recipe');
    if (response.statusCode == 200) {
      return response.data;
    } else {
      throw Exception("/top-chef/recipe so'rovimiz xato ketti!");
    }
  }
